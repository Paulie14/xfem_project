...
int XModel::recursive_output(double tolerance, PersistentTriangulation< 2  >& output_grid, DoFHandler<2> &temp_dof_handler, FE_Q<2> &temp_fe, const unsigned int cycle)
{
  DBGMSG("output\n");
  ...
  XFEValues<Enrichment_method::xfem_shift> xfevalues(fe,quadrature_formula, update_values | update_quadrature_points);
  //XFEValues<Enrichment_method::sgfem> xfevalues(fe,quadrature_formula, update_values | update_quadrature_points);
  ...
  for (; cell!=endc; ++cell)
  {
    ...
    for(unsigned int q=0; q < temp_fe_values.n_quadrature_points; q++)
    {
      ...
      for(unsigned int i=0; i < dofs_per_cell; i++)
      {
        inter += temp_fe_values.shape_value(i,q) * dist_solution[temp_local_dof_indices[i]];
      }
      for(unsigned int w=0; w < cell_xdata->n_wells(); w++)
      {
        for(unsigned int j=0; j < dofs_per_cell; j++)
    ...
    //DBGMSG("difference: %e, cell: %d\n",difference,cell->index());
